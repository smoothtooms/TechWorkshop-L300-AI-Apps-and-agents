...
def get_chat_client() -> FoundryChatClient:
    """Return Foundry chat client with correct token scope for Foundry endpoints."""
    ...
    # FoundryChatClient handles authentication with the correct scope for Foundry endpoints
...
class AgentFrameworkProductManagementAgent:
    ...
    def __init__(self):
        # Configure the chat completion service using Foundry endpoint
        # The client returned by project_client.get_openai_client() is properly authenticated
        # for the Foundry endpoint with correct token scope (https://ai.azure.com)
